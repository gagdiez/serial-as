...
function check_encode<T>(object: T, expected: string): void {
  ...
  let res: string = JSONSerializer.encode(object);

  expect(res).toBe(expected, "failed to encode")
}

function check_decode<T>(encoded: string, original: T): void {
  // Checks that an encoding returns the expected object
  let deco: T = JSONDeserializer.decode<T>(encoded) as T;
  expect(deco).toStrictEqual(original, "failed to decode")
}
